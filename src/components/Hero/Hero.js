...
export default function Hero() {
	...
	return (
		<div className='hero-container'>
			{/* === Section principale (titre + intro) === */}
						...
						<div className='home-skills-grid'>
							{skills.map((skill, index) => (
								<div key={index} className='home-skill-item'>
									<Image src={isDarkMode ? skillIcons[skill].light : skillIcons[skill].dark} alt={skill} width={40} height={40} />
									<span>{skill}</span>
								</div>
							))}
						</div>
			...
			{/* === Section "Teasers" (projets / services) === */}
